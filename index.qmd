...
p <- ggplot(df_summary, aes(x = Year, y = speed, color = Gender)) +
  ...
  theme(
    ...
    axis.title = element_text(size = 15), # Increased size for axis titles
    axis.text = element_text(size = 12),  # Increased size for axis tick labels
    legend.title = element_text(size = 18), # Increased size for legend title
...
# Calculate bottom annotation y-position based on plot height, adjusted for larger text
y_pos_bottom_annotations <- min_y_data + (max_y_data - min_y_data) * 0.08 # Increased slightly for more room

# Add annotations (rectangles for shaded areas, vlines for dotted lines)
# All 'size' values for annotations increased to 5 for better readability.
# Position adjustments (y, vjust, hjust) are made to prevent overlap.
...
p <- p + annotate("rect", xmin = 2008, xmax = 2010.5, ymin = -Inf, ymax = Inf,
  ...
  annotate("text", x = (2008 + 2010.5) / 2, y = y_pos_bottom_annotations,
           ...
           vjust = 0.5, hjust = 0.3, # Centered relative to x,y
           color = "darkgoldenrod", size = 5) # Increased size
...
p <- p + annotate("rect", xmin = 1939, xmax = 1946, ymin = -Inf, ymax = Inf,
  ...
  annotate("text", x = 1942, y = y_pos_bottom_annotations,
           ...
           vjust = 0.5, hjust = -2.9, # Centered relative to x,y
           color = "darkred", size = 5) # Increased size
...
p <- p + annotate("rect", xmin = 1914, xmax = 1919, ymin = -Inf, ymax = Inf,
  ...
  annotate("text", x = 1916, y = y_pos_bottom_annotations,
           ...
           vjust = 0.5, hjust = -3.05, # Centered relative to x,y
           color = "darkblue", size = 5) # Increased size

# Widespread Goggle Use (from ~1970s)
p <- p + geom_vline(xintercept = 1972, linetype = "dotted", color = "darkblue", linewidth = 0.8) +
  annotate("text", x = 1972.5, y = max_y_data * 0.75, # Slightly lower y for more space
           label = "Widespread Goggle Use (from ~1970s)", angle = 90,
           vjust = 1, hjust = 0.85, # Centered relative to x,y
           color = "darkblue", size = 5) # Increased size

# Women’s debut (1912)
p <- p + geom_vline(xintercept = 1912, linetype = "dotdash", color = "pink", linewidth = 0.8) +
  annotate("text", x = 1912.5, y = max_y_data * 0.88, # Slightly lower y for more space
           label = "Women's Debut (1912)", angle = 90,
           vjust = 0.6, hjust = 0.5, # Centered relative to x,y
           color = "deeppink", size = 5) # Increased size

# Standardized Pools (from 1908)
p <- p + geom_vline(xintercept = 1908, linetype = "dotted", color = "purple", linewidth = 0.8) +
  annotate("text", x = 1908.5, y = max_y_data * 0.80, # Slightly lower y for more space
           label = "Standardized Pools (from 1908)", angle = 90,
           vjust = 0.8, hjust = 0.5, # Centered relative to x,y
           color = "#663399", size = 5) # Increased size

# Butterfly Introduction (1956)
p <- p + geom_vline(xintercept = 1956, linetype = "dotted", color = "darkgreen", linewidth = 0.8) +
  annotate("text", x = 1956.5, y = max_y_data * 0.68, # Slightly lower y for more space
           label = "Butterfly Introduced (1956)", angle = 90,
           vjust = 0.5, hjust = 0.7, # Centered relative to x,y
           color = "darkgreen", size = 5) # Increased size
...
<div class="historical-events-box">
## Key Historical Moments in Olympic Swimming

* **Women's Swimming Debut (1912):** The 1912 Stockholm Olympics marked a historic moment as women were officially allowed to compete in swimming events for the first time.
* **WWI (1914-1918) - Olympics Cancelled:** The outbreak of World War I resulted in the suspension of the Olympic Games, pausing international swimming competition.
* **WWII (1939-1945) - Olympics Cancelled:** The global conflict of World War II led to the cancellation of Olympic Games, interrupting competitive swimming's progress.
* **Butterfly Stroke Introduced (1956):** The butterfly stroke was formally recognized as a separate competitive event at the 1956 Melbourne Olympics, showcasing a new, powerful technique.
* **Goggle Use Becomes Widespread (1972):** The widespread adoption of swimming goggles greatly improved visibility and comfort, indirectly aiding performance.
* **Tech-suit Era & Ban (2008-2010):** This period saw a significant, albeit temporary, surge in speeds due to advanced full-body polyurethane swimsuits.
</div>